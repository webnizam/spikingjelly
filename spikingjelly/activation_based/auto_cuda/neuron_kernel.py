...
class NeuronBPTTKernel(base.CKernel2D):
    ...
    def core(self):
        ...
        if self.hard_reset:
            ...
            core_codes.append(
                f"{self.dtype} grad_v_to_h = {cfunction.constant(None, 1., dtype=self.dtype)}"
            )
...
def scalar_to_sycl(py_dict: dict, ref: str = "x_seq"):
    device = py_dict[ref].device
        ...
        :return: requires_grad, blocks, threads, py_dict
            ...
            py_dict: dict
                Compared with the input ``py_dict``, the returned ``py_dict`` will:
                    ...
                    * convert all ``float/int`` scalars in ``py_dict`` to ``dpnp.ndarray``
                    ...
                    * add ``N, numel`` to ``py_dict``. Note that ``x_seq.shape = [T, N]`` and ``numel = T * N``.
                      ...
                      Note that ``N, numel`` in the returned ``py_dict`` are ``dpnp.ndarray``


        :rtype: tuple
        """
        device = py_dict["x_seq"].device
        requires_grad = if_requires_grad(py_dict.values())
        scalar_to_sycl(py_dict)
